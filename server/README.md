...
> **צריך את זה בכלל?** לא בהכרח. השרת הזה הוא אפשרות אחת; אפשרות שנייה
> היא Cloudflare Worker חינמי, שלא צריך מכונה, לא צריך תעודת HTTPS ולא
> צריך תחזוקה. **המשחק מדבר עם שניהם באותו פרוטוקול בדיוק**, ולכן
> ההחלטה היא רק איפה נוח לכם. אם כבר יש worker פרוס — פשוט מדביקים את
> הכתובת שלו במסך ההורים ומדלגים על כל הקובץ הזה.

## הפרוטוקול

שלוש שורות, וזה הכל. הקוד עובר כפרמטר שאילתה:

| בקשה | תשובה |
|---|---|
| `GET /?code=CODE` | `{ "save": … \| null, "updatedAt": "…" \| null }` |
| `POST /?code=CODE` עם השמירה בגוף | `{ "ok": true, "updatedAt": "…" }` |
| כל השאר | `4xx` עם `{ "error": "…" }` |

תא ריק מחזיר `save: null` ולא 404 — זה מה שרואים במכשיר הראשון שמתחבר,
וזו לא שגיאה. גוף **בלי מערך `log` נדחה** ב-`400 not a game save`: זו
הגנה מפני לקוח שבור שימחק שנה של היסטוריה בבקשה אחת.

הצורה הישנה `/save/<code>` ממשיכה לעבוד, כדי שמכשיר עם גרסה ישנה של
המשחק לא יישאר בחוץ עד שירענן.
...
4. **לסנכרן עכשיו**.
5. בכל מכשיר נוסף: אותה כתובת, **אותו קוד**, ושוב לסנכרן עכשיו.
